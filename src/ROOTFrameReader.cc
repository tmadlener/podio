...
podio::CollectionReadBuffers ROOTFrameReader::getCollectionBuffers(ROOTFrameReader::CategoryInfo& catInfo,
  ...
  const auto& [collectionClass, isSubsetColl, index] = catInfo.storedClasses[iColl].second;
  auto& branches = catInfo.branches[index];

  // Create empty collection buffers, and connect them to the right branches
  const auto& bReg = CollectionBufferRegistry::instance();
  auto createFunc = bReg.createBuffers(collectionClass, isSubsetColl);

  // TODO: Error handling of empty optional
  auto collBuffers = createFunc.value_or(podio::CollectionReadBuffers{});
...
createCollectionBranches(TChain* chain, const podio::CollectionIDTable& idTable,
  ...
  for (const auto& [collID, collType, isSubsetColl, collSchemaVersion] : collInfo) {
    ...
    storedClasses.emplace_back(name, std::make_tuple(collType, isSubsetColl, collectionIndex++));
