...
namespace {
   ...
   * CollectionBufferRegistry. Takes the BasicType as template argument.
   *
   * Returns an integer so that it can be used with std::apply
   */
  template <typename T>
  int registerUserDataCollection(T) {
    const auto type = std::string("podio::UserDataCollection<") + userDataTypeName<T>() + ">";
    CollectionBufferRegistry::mutInstance().registerCollType(type, [](bool) {
